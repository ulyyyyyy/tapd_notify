...
type WebhookCfg struct {
	...
	MatchCondition model.JsonListParser `json:"match_condition" gorm:"type:varchar;size:600;column:match_condition"` // 推送匹配条件
	SummaryField   model.StrList        `json:"summary_field" gorm:"type:varchar;size:30;column:summary_field"`      // 概要字段
	PushField      model.StrList        `json:"push_field" gorm:"type:varchar;size:30;column:push_field"`            // 推送字段
	PushList       model.JsonListParser `json:"push_list" gorm:"type:varchar;size:1000;column:push_list"`            // 推送人列表
}
...
func FindByProjectId(projectId string, pageNumber, pageSize int) (wbCfgList []WebhookCfg, err error) {
